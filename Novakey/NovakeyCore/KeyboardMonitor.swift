...
public class KeyboardMonitor {
    ...
    private let conversionDelay: TimeInterval = 0.7
    ...
    private func handleEvent(proxy: CGEventTapProxy, type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if let activeApp = NSWorkspace.shared.frontmostApplication {
            ...
            if type == .keyDown {
                if keyCode == 51 { // バックスペースキー
                    if !currentInput.isEmpty {
                        currentInput.removeLast()
                        lastInputTime = Date()
                    }
                } else if let char = convertKeyCodeToChar(keyCode, flags: flags) {
                    currentInput += char
                    lastInputTime = Date()
        ...
        if let hiragana = convertRomajiToHiragana(currentInput) {
            ...
            Task {
                do {
                    ...
                    // 入力文字列を削除（バックスペースを入力文字数分送信）
                    for _ in 0..<currentInput.count {
                        let backspaceEvent = CGEvent(keyboardEventSource: nil, virtualKey: 0x33, keyDown: true)
                        backspaceEvent?.post(tap: .cghidEventTap)
                        let backspaceUpEvent = CGEvent(keyboardEventSource: nil, virtualKey: 0x33, keyDown: false)
                    ...
                    pasteboard.setString(kanji, forType: .string)
